...
async function loadAccounts(user) {
    ...
    querySnapshot.forEach(docSnap => {
        ...
        card.innerHTML = `
          <div class="Account-Card shadow p-3">
            <div class="d-flex flex-row justify-content-between">
              ...
              <div class="form-check form-switch">
                <input class="form-check-input default-switch" id="${switchId}" type="checkbox"
                  ${account.isDefault ? "checked" : ""} data-id="${accountId}">
              </div>
            </div>
            <p class="Accoun-Balance">₹${account.initialBalance.toFixed(2)}</p>
            ...
            toggleSwitch?.addEventListener("change", async function () {
              if (this.checked) {
                ...
                try {
                  ...
                  switchEl.checked = !originalChecked; // Revert UI
                } finally {
                  // Restore switch UI
...
form.addEventListener('submit', async (e) => {
    ...
    const isDefault = document.getElementById("flexSwitchCheckDefault").checked;
